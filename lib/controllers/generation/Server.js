...
Server.prototype.initHandlers = function(serverConfig){
  this.then(function(){
    ...
    //load sockets server
    this.io = new Socket(this.db, this.controllers);
...
Server.prototype.stop = function (next) {
  ...
  this.then(function(){
    if (!this.serverConfig.status.port){
      console.log('------ Server has no port', this.serverConfig.name);
      return this.next();
    }
    var server = this;
    this.started.close(function(){
      ...
      console.log('------ Express server stopped', server.serverConfig.name);
    });
    this.next();
  });
  return this;
