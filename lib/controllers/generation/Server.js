...
Server.prototype.initDB = function(serverConfig){
  this.then(function(next){
    ...
    this.db = mongoose.createConnection('mongodb://localhost/' + serverConfig.name, { db: { safe:true } });
...
Server.prototype.start = function(port){
  this.then(function(){
    ...
    this.started = this.app.listen(port, function () {
      console.log('++++++ Express server starting', port);
    });
...
Server.prototype.stop = function (next) {
  this.then(function(){
    ...
    this.started.close(function(){
      console.log('------ Express server stopping ', server.serverConfig.status);
      delete server.serverConfig.status.port;
